...
class tendency(QDialog):
    def __init__(self):
        ...
        self.ui.export_btn.clicked.connect(self.exportWordDoc)
        self.ui.export_btn_2.clicked.connect(self.exportWordDoc_2)
    ...
    def plotQuantity(self, condition="1=1"):
        ...
        #query_add = f"SELECT operation_datetime, Quantity FROM object_dist WHERE actions = 'Adding' order by operation_datetime"
        cursor.execute(query_add)
        table_add = cursor.fetchall()
        ss_aaaa_add = []
        activ_add =[]
    ...
    def exportWordDoc(self):
        try:
            ...
            header_text.text = "GC-INO EM Project"
            ...
            p.add_run('wthe tendency').bold = True
            ...
            file_path = os.path.dirname(os.path.dirname(__file__)) + '/run/tendency' + '/' + str(date_time) +'_tendency_'+ name+ '.docx'
            ...
            message_box.setText("The Word file has been saved successfully.")
            message_box.setInformativeText("Click 'Show Details' to open the file.")
            message_box.setDetailedText(file_path)
            message_box.setStandardButtons(QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
            message_box.buttonClicked.connect(lambda button: self.handleMessageBoxButton(button, file_path))
            message_box.exec()
        except Exception as e:
